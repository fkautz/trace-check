// Command trace-check reconciles a requirements catalog against the tests that
// claim to cover it, the waivers that excuse the rest, and an optional
// wire-observability classification — then regenerates a traceability matrix.
//
// It is dialect-agnostic: how requirement IDs are shaped, how the catalog
// markdown is laid out, what tag keyword marks coverage, which languages to
// scan, and how the matrix is labelled all come from a JSON config
// (see -config). With no config it uses built-in defaults; run `trace-check
// -help` for the full description, the config schema, and worked examples.
package main
...
func main() {
	...
	configPath := fs.String("config", "", "path to a JSON dialect config; empty uses built-in defaults (see -help)")
	root := fs.String("root", ".", "repository root scanned for coverage tags")
	catalog := fs.String("catalog", "spec/requirements.md", "requirements catalog path, relative to -root")
	classification := fs.String("classification", "spec/classification.md", "classification path, relative to -root; absent file is dormant")
	waivers := fs.String("waivers", "spec/waivers.md", "waivers path, relative to -root; absent file means no waivers")
	out := fs.String("out", "docs/traceability.md", "matrix output path, relative to -root; empty disables matrix generation")
	outJSON := fs.String("out-json", "", "optional machine-readable matrix path, relative to -root; empty disables")
	architecture := fs.String("architecture", "", "architecture registry path, relative to -root; overrides config architecture.path when set")
	strict := fs.Bool("strict", false, "enforce full coverage: every in-scope requirement needs a tagged test or waiver, and classification/policy coverage-class rules apply")
	strictPhase := fs.String("strict-phase", "", "comma-separated Phase values to include under -strict (e.g. 1 or 1,2); empty uses config strict.phases")
	strictClass := fs.String("strict-class", "", "comma-separated keyword classes to include under -strict (e.g. must); empty uses config strict.keywordClasses")
	profile := fs.String("profile", "", "named policy profile from config profiles{} (may force strict and filters)")
	_ = fs.Parse(os.Args[1:])

	// Accept a bare "help" subcommand (in addition to -help/--help, which the
	// flag package handles), since that is a natural thing to try.
	if fs.NArg() > 0 {
		if fs.Arg(0) == "help" {
			usage(os.Stdout)
			return
		}
		fmt.Fprintf(os.Stderr, "trace-check: unexpected argument %q (this tool takes flags only; run `trace-check -help`)\n", fs.Arg(0))
		os.Exit(2)
	}
	...
	scope := tracecheck.Scope{
		Root:           *root,
		Catalog:        join(*root, *catalog),
		Classification: join(*root, *classification),
		Waivers:        join(*root, *waivers),
		Out:            *out,
		OutJSON:        *outJSON,
		Strict:         *strict,
	}
	if *architecture != "" {
		scope.ArchitecturePath = join(*root, *architecture)
	}
	if *strictPhase != "" {
		scope.StrictPhases = splitCSV(*strictPhase)
	}
	if *strictClass != "" {
		scope.StrictKeywordClasses = splitCSV(*strictClass)
	}
	if err := cfg.ApplyProfile(*profile, &scope); err != nil {
		fmt.Fprintf(os.Stderr, "trace-check: %v\n", err)
		os.Exit(2)
	}
...
// join prefixes a path with root unless it is empty or already absolute.
...
const usageText = `trace-check — requirement traceability checker
