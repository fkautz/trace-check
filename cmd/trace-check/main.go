...
// Module-cache builds do not normally carry vcs.* BuildInfo settings.
var (
	buildVersion  string
...
func main() {
	...
	problemsJSON := fs.String("problems-json", "", "machine-readable diagnostics path, written on clean or failed checks; empty disables")
	checkOutput := fs.Bool("check-output", false, "compare -out/-out-json with generated bytes without writing")
	...
	coversExtra := fs.String("covers-extra-catalog", "", "comma-separated additional catalog paths (relative to -root) whose IDs are valid structured-Covers targets, for cross-scope covered-by; not loaded as requirements")
	...
	if *showVersion {
		fmt.Fprintln(os.Stdout, formatBuildVersion(buildInfo))
		return
	}
	...
	scope := tracecheck.Scope{
		...
		ToolVersion:    formatBuildVersion(buildInfo),
		Strict:         *strict,
	}
	if *configPath != "" {
		absoluteConfig, err := filepath.Abs(*configPath)
		if err != nil {
			fmt.Fprintf(os.Stderr, "trace-check: resolve config path: %v\n", err)
			os.Exit(2)
		}
		scope.ConfigPath = absoluteConfig
	}
	if *architecture != "" {
		scope.ArchitecturePath = join(*root, *architecture)
	}
	if *coversExtra != "" {
		for _, p := range splitCSV(*coversExtra) {
			scope.CoversExtraCatalogs = append(scope.CoversExtraCatalogs, join(*root, p))
		}
	}
	if *strictPhase != "" {
...
func formatBuildVersion(info *debug.BuildInfo) string {
	...
	return fmt.Sprintf("trace-check version=%s revision=%s modified=%s", version, revision, modified)
}
